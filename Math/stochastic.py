...
train_data = data.head(int(data_length * 0.9))[["G1", "G2", "G3"]]
...
def partial_derivative_of_costfunction(thetas, j, x1, x2, y):
    xs = [1, x1, x2]
...
def update_thetas(thetas, epochs, learning_rate):
    ...
    for x in range(len(train_data)):
        ...
        for j in range(len(thetas)):
            change = partial_derivative_of_costfunction(thetas, j, data_j0[x], data_j1[x], data_ys[x])
