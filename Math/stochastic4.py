...
train_data = data.head(int(data_length * 0.9))[["G1", "G2", "G3", "absences", "freetime"]]
data_j0 = train_data["G1"]
data_j1 = train_data["G2"]
data_j2 = train_data["absences"]
data_j3 = train_data["freetime"]
...
def partial_derivative_of_costfunction(thetas, j, x1, x2, x3, x4, y):
    xs = [1, x1, x2, x3, x4]
    hypothesis = get_hypothesis(thetas, xs)
    err = error(hypothesis, y)
    sum = err * xs[j]
...
def update_thetas(thetas, epochs, learning_rate):
    ...
    for x in range(len(train_data)):
        ...
        for j in range(len(thetas)):
            change = partial_derivative_of_costfunction(thetas, j, data_j0[x], data_j1[x], data_j2[x], data_j3[x], data_ys[x])
...
def find_thetas():
    ...
    thetas = [0,0,0,0,0]
    thetas = update_thetas(thetas, 10000, 0.01)
